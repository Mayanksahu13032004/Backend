...
const logoutUser=asyncHandler(async(req,res)=>{
            await User.findByIdAndUpdate(
                ...
                {
                    $unset:{
                        // this  remove filed from document
                        refreshToken:1
                    }
                },
                {
...
.json(new ApiResponse(200,user,"Account details succcessfully"))
...
const updateUserAvatar=asyncHandler(async(req,res)=>{
   ...
   const avatar=await uploadOnCloudinary(avatarLocalPath)
   if (avatar.url) {
    throw new ApiError(400,"Error while uploaading on avatar")
   }
//    avatar update ho raha hai 
   const user= await User.findByIdAndUpdate(
    req.user?._id,
    {
        $set:{
   ...
   return res.status(200)
    .json(
        new ApiResponse(200,user,"Avartar Image updated successfully")
    )
})
...
const updateUserCoverImage=asyncHandler(async(req,res)=>{
    ...
    const coverImage=await uploadOnCloudinary(coverImageLocalPath)
    if (coverImage.url) {
     throw new ApiError(400,"Error while uploaading on coverimage")
    }
 //    avatar update ho raha hai 
    const user= await User.findByIdAndUpdate(
     req.user?._id,
